...
parse_gettoken(parsestate_t *parsestate, token_t *token)
{
	...
	tokentype_t type = TOK_NORMAL;
	char prev_c = '\0';

	int quote_state = 0;			// 0: closed; 1: open quote
	int any_quotes = 0;			// 0: no quotes; 1: otherwise
	int QUOTE_OPEN = 1;
	int QUOTE_CLOSED = 0;
	...
	while (*str != '\0') {
		if (i >= TOKENSIZE - 1)
			...
			goto error;
		if(quote_state == QUOTE_CLOSED) {
		  // not open quote state
		  // break upon isspace()	    
		  // and enter QUOTE_OPEN upon '\"'
		  // also, check the token type here

		  // check the token type
		  type = check_tokentype(prev_c, *str, type);
		  ...
		  // open quote state, ignore space
		  // search for close quote, and change
		  // the quote state upon finding
		  if(*str == '\"'){
		    any_quotes ++;
		    quote_state = QUOTE_CLOSED;
		  }
		}
		// save current char and move ptr forward
		prev_c = *str;  // save current c to prev_c before moving
		token->buffer[i++] = *str++;
	}
...
/*
 * get_tokentype(char, char, tokentype_t)
 *
 *   get the tokentype based on prev char, current char
 *   and the current tokentype
 */
toketype_t get_tokentype(char prev_c, char c, tokentype_t current_type) {
  tokentype_t type = TOK_NORMAL;
  switch(c){
  case '<':
    type = TOK_LESS_THAN;
  ...
  // check whether there is conflict in new type
  // and current type
  if(current_type == TOK_NORMAL)
    return type;
  if(type == TOK_NORMAL)
    return current_type;
  if(current_type == type)
    return type;
  if((current_type == TOK_AMPERSAND && type == TOK_DOUBLEAMP) ||
     (current_type == TOK_PIPE && type == TOK_DOUBLEPIPE))
    return type;
  // there is conflict
  return TOK_ERROR;
}
...
command_parse(parsestate_t *parsestate)
{
	...
	while (1) {
		...
		switch (token.type) {
		case TOK_NORMAL:
		  // check whether argv reach MAXTOKENS
		  if(i < MAXTOKENS){
		    cmd->argv[i] = strdup(token.buffer);
		    i++;
		  }
		  else{
		    // TODO: too many tokens, reject the cmd
		  }
		  break;

		case TOK_LESS_THAN:
		  // get next token, which is the redirection file name
		  parse_gettoken(parsestate, &token);
		  if(token.type != TOK_NORMAL)
		    // next token is not normal, return error
		    goto error;
